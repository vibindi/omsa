...
class Graph:
    ...
    def add_edge(self, source: str, target: str) -> None:
        ...
        """
        if (not (source, target) in self.edges) and (not (target, source) in self.edges):
    ...
    def max_degree_nodes(self) -> dict:
        ...
        for id, _ in self.nodes:
            res[id] = sum([1 for e in self.edges if e[0] == id or e[1] == id])
        return {k:v for k,v in res.items() if v == max(res.values())}
